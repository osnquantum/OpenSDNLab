...
from engine.analysis.qos.qoe_classifier import classify_mos
...
class AnalyticsService:
    ...
    def experiment_analysis(self, name):
        ...
        if not rows:
            ...
            return {
                "samples": 0,
                "rtt": {"mean": 0, "median": 0, "min": 0, "max": 0, "std": 0},
                "jitter": {"mean": 0, "std": 0},
                "throughput": {"mean": 0, "min": 0, "max": 0},
                "packet_loss": {"mean": 0},
                "one_way_delay": {"mean": 0},
            }
        ...
        return {
            ...
            "rtt": {
                ...
                "min": min(rtt),
                "max": max(rtt),
                "std": statistics.stdev(rtt) if len(rtt) > 1 else 0,
            },
            "jitter": {
                "mean": statistics.mean(jitter),
                "std": statistics.stdev(jitter) if len(jitter) > 1 else 0,
            },
            "throughput": {
            ...
            "packet_loss": {"mean": statistics.mean(loss)},
            "one_way_delay": {"mean": statistics.mean(delay)},
            "mos": {
                "mean": statistics.mean(mos),
                "min": min(mos),
                "max": max(mos),
                "quality": classify_mos(statistics.mean(mos)),
            },
        }
